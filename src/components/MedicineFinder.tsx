    ...
    <div className="space-y-6 pb-12">
        ...
        <div className="relative z-10 max-w-3xl">
          ...
          <div className="mt-6 relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-12 py-3.5 sm:py-4 bg-white text-slate-900 placeholder:text-slate-400 rounded-2xl shadow-lg text-sm sm:text-base font-semibold focus:outline-none focus:ring-4 focus:ring-emerald-400/50"
            />
            {searchQuery && (
              <button
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Rack:</label>
          <select
            value={selectedRack}
            onChange={(e) => setSelectedRack(e.target.value)}
            className="text-xs font-bold bg-white border border-slate-200 rounded-xl px-3 py-1.5 text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500"
          >
            {uniqueRacks.map(r => (
              <option key={r} value={r}>{r === 'All' ? 'All Racks' : r}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-xs text-slate-500 px-1">
        <p>
          Showing <span className="font-bold text-slate-800">{filteredMedicines.length}</span> medicines found
        </p>
        <button
          onClick={onOpenPos}
      ...
      {filteredMedicines.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-slate-200 shadow-sm">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
            We couldn't find any medicine matching "<span className="font-semibold">{searchQuery}</span>". Try another spelling or check the full inventory.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {filteredMedicines.map((med) => {
            ...
            return (
              <div
                key={med.id}
                className="bg-white rounded-2xl border border-slate-200 hover:border-emerald-300 hover:shadow-lg transition-all p-5 flex flex-col justify-between group"
              >
                <div>
                  ...
                  <div className={`p-3 rounded-xl mb-3 flex items-start justify-between ${
                    ...
                    <div className="flex items-start gap-2.5">
                      <div className={`p-2 rounded-lg ${isColdStorage ? 'bg-blue-200 text-blue-800' : 'bg-emerald-200 text-emerald-800'}`}>
                      ...
                      <div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <span className="font-bold text-xs text-slate-800 bg-white/80 px-2 py-0.5 rounded shadow-2xs">
                            Shelf {med.rackLocation.shelfNumber}
                          </span>
                          {med.rackLocation.boxNumber && (
                            <>
                              <span className="text-slate-400 font-bold">›</span>
                              <span className="font-mono font-bold text-xs text-slate-700 bg-white/80 px-1.5 py-0.5 rounded">
                                {med.rackLocation.boxNumber}
                              </span>
                            </>
                          )}
                    ...
                    <button
                      ...
                      className="text-slate-400 hover:text-emerald-700 p-1 hover:bg-white/60 rounded"
                    >
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <h3 className="text-base font-extrabold text-slate-900 mt-1.5 group-hover:text-emerald-800 transition-colors">
                        {med.name}
                      </h3>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-slate-100 text-xs">
                    <div>
                      ...
                      <span className="font-semibold text-slate-700">{med.dosage} ({med.stripSize})</span>
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-400 uppercase font-bold block">Batch / Expiry</span>
                      <span className="font-mono text-slate-700">{med.batchNumber} • {med.expiryDate.slice(0, 7)}</span>
                    </div>
                  </div>

                  {/* Temperature Storage note */}
                  {med.rackLocation.temperatureNote && (
                    <div className="mt-2.5 flex items-center gap-1.5 text-[11px] text-slate-600 bg-slate-50 px-2.5 py-1 rounded-lg">
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                  <div>
                    <div className="flex items-baseline gap-1">
                      ...
                      <span className="text-xl font-black text-slate-900">{med.unitPrice.toFixed(2)}</span>
