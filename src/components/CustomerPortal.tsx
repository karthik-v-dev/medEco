...
import { Customer, Invoice, MedicineReminder, OnlineOrder } from '../types';
import { ReminderManager } from './ReminderManager';
import { getCustomerInvoices, getCustomerReminders, getCustomerOnlineOrders, getPharmacyBranches } from '../services/firebase';
      ...
      <div className="max-w-lg mx-auto my-12 bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-md text-center space-y-4">
        ...
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto">
            Log in with your mobile phone number to view your full medicine purchase history, track live online bookings, and manage daily dose reminders.
          </p>
        </div>
        ...
        <p className="text-[11px] text-slate-400">
          Try demo phone: <strong className="text-slate-700 dark:text-slate-300 font-mono">9876543210</strong> (Rahul Sharma)
        </p>
      </div>
    );
  }
          ...
          {customerOrders.length === 0 ? (
            <div className="bg-white dark:bg-slate-900 p-12 rounded-3xl border border-slate-200 dark:border-slate-800 text-center space-y-3">
              ...
              <p className="text-xs text-slate-500 max-w-sm mx-auto">
                Need medicines delivered to your doorstep? Upload your doctor prescription or select tablets from our storage racks.
              </p>
              {onOpenOnlineOrder && (
                <button
                  onClick={onOpenOnlineOrder}
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-800 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950 px-2.5 py-1 rounded-xl">
                          ...
                          <span>Fulfilling Store: <strong>{order.pharmacyName || branch?.name || 'Koramangala 5th Block'}</strong></span>
                        </div>
                  ...
                  {inv.items.map((item, idx) => (
                    <div key={idx} className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 flex items-start justify-between">
                      <div>
                        ...
                        <span className="text-[9px] font-mono text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950 px-1.5 py-0.2 rounded mt-1 inline-block">
